...
pub(crate) fn k_cos(x: f64, y: f64) -> f64 {
    ...
    let hz = 0.5 * z;
    verifier_equiv_check_f64(hz, 3);
    let w = 1.0 - hz;
    verifier_equiv_check_f64(w, 4);
    verifier_equiv_check_f64(w + (((1.0 - w) - hz) + (z * r - x * y)), 5);
    w + (((1.0 - w) - hz) + (z * r - x * y))
}

extern "C" {
    fn __kernel_cos(x: f64, y: f64) -> f64;
}
...
fn main() {
    ...
    verifier_assume!(!y.verifier_is_nan());
    verifier_assume!(!y.verifier_is_infinite());
    let z2 = unsafe { __kernel_cos(x, y) };
    let z1 = k_cos(x, y);
