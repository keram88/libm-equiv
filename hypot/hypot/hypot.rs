...
extern "C" {
    fn fake_sqrt(x: f64) -> f64;
}

fn rust_sqrt(x: f64) -> f64 {
    unsafe { fake_sqrt(x) }
}
...
pub fn rust_hypot(mut x: f64, mut y: f64) -> f64 {
    ...
    let (hx, lx) = sq(x);
    let (hy, ly) = sq(y);
    z * rust_sqrt(ly + lx + hy + hx)
}

extern "C" {
    fn musl_hypot(x: f64, y: f64) -> f64;
}
...
fn main() {
    ...
    let h1 = rust_hypot(x, y);
    let h2 = unsafe { musl_hypot(x, y) };
